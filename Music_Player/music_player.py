...
from PyQt5.QtWidgets import QPushButton, QVBoxLayout, QWidget, QHBoxLayout, QSlider, QLabel, QAction, QFileDialog, \
...
class MainWindowMusicPlayer(QMainWindow):
    ...
    def home_screen(self):
        ...
        self.setWindowIcon(QIcon('icons\\music.png'))
    ...
    def play_handler(self):
        self.userAction = 1
        self.statusBar().showMessage(f'Playing at Volume {self.player.volume()}')
        if self.player.state() == QMediaPlayer.StoppedState:
            ...
            elif self.player.mediaStatus() == QMediaPlayer.LoadedMedia or self.player.mediaStatus() == QMediaPlayer.BufferedMedia:
                self.player.play()
    ...
    def pause_handler(self):
        self.userAction = 2
        self.statusBar().showMessage(f'Paused {self.player.metaData(QMediaMetaData.Title)} \
                                        at position {self.centralWidget().layout().itemAt(0).layout().itemAt(0).widget().text()} \
                                        at Volume {self.player.volume()}')
        self.player.pause()

    # Music stop function
    def stop_handler(self):
        self.userAction = 0
        self.statusBar().showMessage(f'Stopped at Volume {self.player.volume()}')
    ...
    def qmp_media_status_changed(self):
        if self.player.mediaStatus() == QMediaPlayer.LoadedMedia and self.userAction == 1:
            ...
            self.centralWidget().layout().itemAt(0).layout().itemAt(2).widget().setText('%d:%02d' % (int(durationT / 60000), 
    ...
    def open_file(self):
        file_Chosen = QFileDialog.getOpenFileUrl(self, 'Open Music File', expanduser('~'), \
    ...
    def add_files(self):
        folder_Chosen = QFileDialog.getExistingDirectory(self, 'Open Music Folder', expanduser('~'))
